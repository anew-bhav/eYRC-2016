...
destinations=[[126,217],[58,300],[126,217],[205,334]]
...
'''def bot_position(hsv):
    ...
    bot_lower = np.array([0,159,92])                # Lower HSV value for bot_markers
    bot_upper = np.array([11,255,255])             # Higher HSV value range for bot_markers
    ...
    contours, hierarchy = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)      # find contour of masked colour
    contours = sorted( contours, key = cv2.contourArea, reverse = True)[:3]                    # sort contours decreasing order of area and store only 2 
...
def orientmove(slope_bot2cell,slope_botmarkers,distance_centre2cell,distance_other2cell,bot_distance):
        ...
        if distance_other2cell < 20 :                                       # check slave bot reaches destination if yes stop
                ser.write("\xfd")
                print "Reached Near destination"
                return 1
...
if __name__ == "__main__":
    ...
    while destinations != []:
        destination_position[0] = destinations.pop()        
        while 1:
                ...
                Bot_position=Bot_pos(hsv)                                 # call bot_position function    
                ...
                if(orientmove(destination_slope,bot_slope,distance_centre2destination,distance_other2destination,bot_distance)==1):
                        ser.write('\xB0')
                        time.sleep(1)
                        ser.write('\xB1')
